...
class DataExplorerService(ABC):
    ...
    def explore_by_similarity(self, image: Union[np.ndarray, List[np.ndarray]],
        ...
        return self.__transform_explorer_dataframe__(self.explorer.get_similar(img=image))

    def explore_by_query(self, query: str, split: Literal["train", "test", "valid"] = "train") -> pd.DataFrame:
        self.explorer.create_embeddings_table(force=True, split=split)
        return self.__transform_explorer_dataframe__(self.explorer.sql_query(query=query))

    def explore_by_prompt(self, prompt: str, split: Literal["train", "test", "valid"] = "train") -> pd.DataFrame:
        self.explorer.create_embeddings_table(force=True, split=split)
        return self.__transform_explorer_dataframe__(self.explorer.ask_ai(query=prompt))

    @classmethod
    def __transform_explorer_dataframe__(cls, explorer_dataframe: pd.DataFrame) -> pd.DataFrame:
        # TODO: Implement a method to transform the explorer dataframe into a more user-friendly format
        return explorer_dataframe
